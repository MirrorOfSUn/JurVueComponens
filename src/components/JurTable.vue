<template>
  ...
  <table class="table table-hover">
    <thead :class="{ [config.colors.thead]: config.colors.thead }">
      <tr>
    ...
    <tbody>
      <template v-for="(obj, index) in tbl.pageDataFiltered()" :key="index">
        ...
        <tr v-if="tbl.select.isSelected(index)">
          <td
            :colspan="tbl.fields.length + (tbl.isShowDetail ? 1 : 0) + (tbl.isBulk ? 1 : 0)"
            class="td-details bg-primary.bg-gradient"
          >
            <slot api="tbl.config.api" pkey="zzz"></slot>
          </td>
        </tr>
      </template>
    </tbody>
